...
There are some terms used, when exploring the topic of virtualization, that are important to clear up. The **host** is the computer being used to run the virtualization program in question (e.g., the `chroot` Unix command, Docker, Oracle VM VirtualBox). The **guest** is the system being run within the virtualization program. For example, I am presently typing this glossary section on Arch Linux, so if I fire up a CentOS 7 VM in Oracle VM VirtualBox, then my host operating system would be Arch Linux and my guest operating system would be CentOS 7. 
