...
DNF, however, is capable of downloading `.src.rpm` packages, which can be extracted (several ways this can be done, may personal preference as it works on any Linux platform is {% include Code/codeus.html line1="bsdtar -xf $package.src.rpm" %}), modified and rebuilt if the user really wants to customize the package, like adjust its configure options. I would personally recommend that if you wish to do this and you are not familiar with the process of building RPM packages that you read [this](http://fedoraproject.org/wiki/How_to_create_an_RPM_package) *Fedora Wiki* article on it.
