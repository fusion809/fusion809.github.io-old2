...
<ul>
  ...
  <li>{% capture point2 %}Compile the source code, by running: {% include Code/codeus.html line1="script/build" %} from within the `atom` directory (to get into it run `cd atom`). This step requires a reliable Internet connection, as otherwise it will fail and you need to re-run this step.{% endcapture %}{{ point2 | markdownify }}</li>
