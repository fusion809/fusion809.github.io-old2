...
By default Jekyll has the following directory structure:
{% include Code/gist.html id="838d04bda42897fd7167" %}
where `()` are optional folders or files that are not included by default, `{}` denotes folders or files generated by Bundler and `[]` denotes folders or files I would recommend is listed in `.gitignore`.
