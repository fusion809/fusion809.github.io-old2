...
(2) Are easy to get and run without any major issues on all my favourite Linux distributions. I usually stick to the following distributions: Arch Linux, Fedora, Funtoo Linux, Gentoo Linux, Manjaro Linux, openSUSE Tumbleweed, Sabayon Linux and Ubuntu. Due to the diversity in my preferred set of distributions it is a fair bet than any game that runs on all of these distributions will likely run on any major distribution. The Steam platform may aid in this if the game is present in their game store, although I have had issues getting Steam to run on Ubuntu 14.04 LTS, for example. Java games are good for this, in theory, but they usually have difficulty fulfilling criterion 4. 

In the game monographs to come I will mention the availability of each game on the major, sufficiently independent Linux distributions. By "sufficiently independent" I mean distributions that have repositories that contain their own, very much unique selection of packages. Manjaro Linux I would not say has "sufficiently independent" repositories as most packages in their repositories come from either the Arch User Repository or Arch Linux official repositories. Sabayon Linux does meet this requirement of being "sufficiently independent" as Sabayon Linux packages come from the Gentoo Portage tree (and NOT all packages in the Portage tree are in the Sabayon repositories), along with the `sabayon` and `sabayon-distro` overlays. 
